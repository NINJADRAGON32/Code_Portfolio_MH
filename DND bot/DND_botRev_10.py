...
async def on_message(message):
    ...
    if content.startswith("!help"):
        embed = discord.Embed(
            title="🎲 DiceBot3000 Help Menu",
            description="Commands for combat, rolling, tracking, and utilities.",
        ...
        embed.add_field(
            ...
            value="`!stats [name] [race]`\n`!npc`",
            inline=False
        )

        embed.set_footer(text="DiceBot3000 • D&D Assistant")
    ...
    # add to the initiative/turn order
        ...
        try:
            _,_,player,roll,dex_mod = message.content.strip().split()
            if combat_active:
            ...
            await message.channel.send("to use type: !init add [name] [dex_mod] [roll]")
    ...
    # combat show 
    elif content.startswith("!combat"):
        if not combat_active:
            await message.channel.send("Combat is not active.")
            return

        msg = f"⚔️ **Combat Tracker**\n"
        ...
        for i, combatant in enumerate(initiative_order):
            ...
            if i == current_turn_index:
                msg += f"{i+1}. {name} ({init}) ← **CURRENT TURN**\n"
            else:
                msg += f"{i+1}. {name} ({init})\n"
        ...
        if not status:
            ...
            for player, effects in status.items():
                effect_list = []

                for effect in effects:
                    condition = effect["condition"]
                    turns = effect["turns"]
                    effect_list.append(f"{condition} ({turns})")

                msg += f"{player}: {', '.join(effect_list)}\n"
        ...
        if not concentration:
            ...
            for player, spell_data in concentration.items():
                spell = spell_data["spell"]
                rounds = spell_data["rounds"]

                msg += f"{player} → {spell} ({rounds} rounds)\n"
    ...
    # next turn command
        ...
        # status turn functionality
        player = initiative_order[current_turn_index]["name"]

        if player in status:
            updated = []

            for effect in status[player]:
                effect["turns"] -= 1

                if effect["turns"] > 0:
                    updated.append(effect)
                else:
                    await message.channel.send(f"{player}'s {effect['condition']} has ended.")

            if updated:
                status[player] = updated
            else:
                del status[player]

        
        # decrement concentration spells
        expired = []
        for player in concentration:
            concentration[player]["rounds"] -= 1
            if concentration[player]["rounds"] <= 0:
                expired.append(player)

        for player in expired:
            await message.channel.send(f"{player}'s concentration on {concentration[player]['spell']} has ended.")
    ...
    # init remove
    elif content.startswith("!init remove"):
        ...
        try:
            ...
            for combatant in initiative_order:
                if combatant["name"].lower() == player.lower():
                    initiative_order.remove(combatant)
                    found = True

                    await message.channel.send(f"Removed **{combatant['name']}** from initiative.")
                    break

            if not found:
                await message.channel.send("Player is not in initiative.")

        except ValueError:
            await message.channel.send("Usage: !init remove [name]")
    ...
    # add a status
        ...
        try:
            _,_,player,condition,turns = message.content.strip().split()
            turns = int(turns)
            
            if player not in status:
                status[player] = []
            
            if any(effect["condition"] == condition.lower() for effect in status[player]):
                await message.channel.send(f" {player} already has the **{condition}** condition.")
            else: 
                status[player].append({
    ...
    # remove status
        ...
        try:
            ...
            if player not in status:
                await message.channel.send(f"{player} has no conditions.")
                return
            ...
            for effect in status[player]:
                if effect["condition"] == condition.lower():
                    removed = True
                else:
                    updated.append(effect)
            ...
            if removed:
                ...
                await message.channel.send(f"{player} is no longer **{condition}**.")
            else:
                await message.channel.send(f"{player} is not **{condition}**.")

        except ValueError:
            await message.channel.send("Usage: !status remove [player] [condition]")
        ...
        if not status:
            ...
            for player, effects in status.items():
                ...
                for effect in effects:
                    condition = effect["condition"]
                    turns = effect["turns"]
                    effect_list.append(f"{condition} ({turns} turns)")
    ...
    # setting the concentration
        ...
        try:
            _,_, player,rounds,spell = message.content.strip().split(maxsplit=4)
            rounds = int(rounds)
    ...
    #showing concentration
        ...
        if not concentration:
            ...
            for player, data in concentration.items():
                msg += f"• {player} → {data['spell']} ({data['rounds']} rounds)\n"
    ...
    # roll for stats ---------------------------------------------------------------------------------------------
        ...
        try:
            _, name, race = message.content.strip().split(maxsplit=3)
